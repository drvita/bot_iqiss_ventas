    ...
    async itemInfo(ctx) {
        ...
        try {
            ...
            if (iqiss && countItems) {
                ...
                fs.appendFile(
                    "items_notfound",
                    Date.now() + `[Buscar] ${decodeURI(search)}`,
                    (err) => {
                        console.log(err);
                    }
                );
            ...
            fs.appendFile(
                "log",
                Date.now() + "- [Buscar] Error catcheado -" + e + "\n",
                (err) => {
                    console.log(err);
                }
            );
            return ctx.reply.text(
                `La conexion no esta disponible, puedes buscar directamente en <a href="${urltienda}">Tienda</a>`, { parseMode: "HTML" }
            );
        }
    }
