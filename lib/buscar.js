    ...
    try {
      ...
      if (!search) {
        return ctx.reply.text(
          "Despues de comando /buscar, escriba el nombre del producto a enviar.",
          { asReply: true }
        );
      }
      ...
      const iqiss = await fetch(url)
          ...
          .catch((err) => {
            fs.appendFile(
              "log",
              Date.now() + `Error al recuperar datos de API: ${err}\n`,
              (err) => {
                console.log(err);
              }
            );
      ...
      await ctx.reply.text(`Articulos encontrados: ${countItems}`, {
        asReply: true,
      });
      ...
      if (iqiss && countItems) {
        ...
        fs.appendFile(
          "log",
          Date.now() + `El producto ${search} no existe en la base de datos\n`,
          (err) => {
            console.log(err);
          }
        );
