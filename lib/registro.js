    ...
    try {
      if (ctx.chat.id !== ctx.from.id) {
        return ctx.reply.text(
          "El registro se hace en privado, da click aqui -> @iqissAdminBot",
          { asReply: true }
        );
      }
      ...
      if (users.find((iduser) => parseInt(iduser) === ctx.from.id)) {
        return ctx.reply.text("Usted ya realizo este registro");
      }
      ...
      if (this.userArray[ctx.from.id] === undefined) {
        ...
        return ctx.reply.text(
          "¡Hola!, para iniciar escribe tu nombre completo."
        );
      } else {
        await ctx.reply.text("Aun tiene un registro pendiente...", {
          asReply: true,
        });
        if (this.userArray[ctx.from.id].status === "name") {
          ...
          return ctx.reply.text(
            "Por favor tecle su numero de telefono a 10 digitos"
          );
    ...
    try {
      if (this.userArray[ctx.from.id] !== undefined) {
        ...
        if (this.userArray[ctx.from.id].status === "name") {
          ...
          return ctx.reply.text(
            `¡Muy bien ${ctx.text.toLowerCase()}!, ahora tu numero de telefono a 10 digitos.`
          );
          ...
          return ctx.reply.text(
            `${this.userArray[ctx.from.id].value.name}, ahora tu e-mail.`
          );
        } else if (this.userArray[ctx.from.id].status === "email") {
          ...
          ctx.reply.text(
            `Muchas gracias ${
              this.userArray[ctx.from.id].value.name
          ...
          return ctx.reply.text(
            `Visitanos en https://tienda.iqissmexico.com.mx`
          );
        }
      }
    } catch (err) {
      ...
      fs.appendFile(
        "log",
        Date.now() + `-[takeDataUSer] error - ${err}\n`,
        (err) => {
          console.log(err);
        }
      );
